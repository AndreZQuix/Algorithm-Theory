...
public:
	...
	multimap<INDIVIDUAL_TYPE>* createPopulation()	// get map of random individuals (population)
	{
		...
		for (int i = 0; i < MAX_POPULATION_SIZE; i++)
		{
			...
			WEIGHT_TYPE pathWeightWithLoop = getPathWeight(temp) + getEdgeByVertexes(temp[0], temp[temp.size() - 1])->getWeight();
			population->insert(pair<INDIVIDUAL_TYPE>(pathWeightWithLoop, temp));	// add individual to the population
		}
		return population;
	}
	...
	PAIR_TYPE* crossingOver(const PAIR_TYPE& parents) {
		...
		for(int counter = 0; counter < 2; counter++)	// crossing over goes two times for two childs
		{
			...
			child.first = getPathWeight(child.second) + getEdgeByVertexes(child.second[0], child.second[child.second.size() - 1])->getWeight();	// calculate child fitness (path weight)
	...
	void swapGnome(pair<INDIVIDUAL_TYPE>* child) {
		...
		child->first = getPathWeight(child->second) + getEdgeByVertexes(child->second[0], child->second[child->second.size() - 1])->getWeight();
	}
...
int main()
{
	// in this algorithm implementation user cannot choose where salesman starts his way (but this is quite easy task for implementation)
